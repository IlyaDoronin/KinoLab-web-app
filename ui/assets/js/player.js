...
const sliders = document.querySelector(".progressbar");
...
document.querySelector("#start").addEventListener("keydown", useHotKeys);
document.querySelector("#volume").addEventListener("keydown", useHotKeys);
document.querySelector("#screen").addEventListener("keydown", useHotKeys);
...
function changeSound(up) {
    ...
    if (up) {
        if (volume <= 0.95) video.volume = volume + 0.05;
        else video.volume = 1;
        volumeImg.setAttribute("src", "/img/player/volume_on.svg");
    } else {
        if (volume > 0.05) video.volume = volume - 0.05;
        else {
            video.volume = 0;
...
function changeSpeed(speed) {
    ...
    if (pbRate > 0.15 && pbRate < 5) video.playbackRate += speed;
    else if (pbRate > 0) video.playbackRate += speed;
    let time = Math.round((pbRate + speed) * 100) / 100;
    time = time.toString().length == 1 ? `${time}.0` : time;
    curSpeed.innerHTML = time;
    showInfo(`Скорость: ${time}x`);
}
...
function progressUpdate() {
    ...
    // Обновление времени
    document.querySelector("#curren-time").innerHTML = toNormalTime(
        video.currentTime,
    );
}
...
function videoRewind() {
    pBtn.style.transition = '0s'
    pDuration.style.transition = '0s'
    ...
    startImg.setAttribute("src", "/img/player/pause.svg");
    setTimeout( () => {
        pBtn.style.transition = '.25s'
        pDuration.style.transition = '.25s'
    }, 10)
}
...
function takeMetaData() {
    ...
    document.querySelector("#full-time").innerHTML = toNormalTime(
        video.duration,
    );
}
